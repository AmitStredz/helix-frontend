...
export function VaultManagement() {
  ...
  const strategies = [
    { name: 'Compound Lending', risk: 'Low', apy: '8-12%', description: 'Lend assets on Compound protocol' },
    { name: 'Uniswap LP', risk: 'Medium', apy: '15-25%', description: 'Provide liquidity to Uniswap pools' },
    { name: 'Cross-Chain Arbitrage', risk: 'High', apy: '20-35%', description: 'Arbitrage opportunities across chains' },
    { name: 'Curve LP + Convex', risk: 'Low', apy: '12-18%', description: 'Stable LP with boosted rewards' }
  ];
                ...
                <Select>
                  ...
                  <SelectContent className="glass-card border-white/10">
                    {strategies.map((strategy, index) => (
                      <SelectItem key={index} value={strategy.name}>
        ...
        <Card className="glass-card">
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {vaults.map((vault) => (
                <Card key={vault.id} className="glass-card hover:bg-card/30 transition-colors">
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      ...
                      <Badge 
                        variant="outline" 
                        className={`${
                          vault.risk === 'Low' ? 'text-success border-success/20' :
                          vault.risk === 'Medium' ? 'text-accent border-accent/20' :
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="font-semibold text-success">{vault.apy}%</span>
                    </div>
                    
                    {/* TVL */}
                    <div className="flex items-center justify-between">
                      <span className="text-sm text-muted-foreground">TVL</span>
                      <span className="font-semibold">{formatCurrency(vault.tvl)}</span>
                    </div>
